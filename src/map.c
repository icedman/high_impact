...
void map_set_anim_with_len(map_t *map, uint16_t tile, float frame_time,
  ...
  if (!map->anims) {
    int animSize = sizeof(anim_def_t *) * map->max_tile;
    map->anims = malloc(animSize);
    memset(map->anims, 0, animSize);
  }

  map_anim_def_t *def =
      malloc(sizeof(map_anim_def_t) + sizeof(uint16_t) * sequence_len);
...
int map_tile_at(map_t *map, vec2i_t tile_pos) {
  if (tile_pos.x < 0 || tile_pos.x >= map->size.x || tile_pos.y < 0 ||
      tile_pos.y >= map->size.y) {
    return 0;
  } else {
