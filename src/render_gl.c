...
#define bind_va_f(index, container, member, start)                             \
  glVertexAttribPointer(index, member_size(container, member) / sizeof(float), \
                        GL_FLOAT, false, sizeof(container),                    \
                        (GLvoid *)(offsetof(container, member) + start))

#define bind_va_color(index, container, member, start)                         \
  glVertexAttribPointer(index, 4, GL_UNSIGNED_BYTE, true, sizeof(container),   \
                        (GLvoid *)(offsetof(container, member) + start))
...
static const char *const SHADER_GAME_VS = SHADER_SOURCE_VS(
    IN vec2 pos; IN vec2 uv; IN vec4 color; OUT vec4 v_color; OUT vec2 v_uv;

    uniform vec2 screen; uniform vec2 fade; uniform float time;

    void main(void) {
      v_color = color;
      v_uv = uv;
      gl_Position =
          vec4(floor(pos + 0.5) * (vec2(2, -2) / screen.xy) + vec2(-1.0, 1.0),
               0.0, 1.0);
    });

static const char *const SHADER_GAME_FS =
    SHADER_SOURCE_FS(IN vec4 v_color; IN vec2 v_uv;
...
prg_game_t *shader_game_init(void) {
  prg_game_t *s = malloc(sizeof(prg_game_t));
...
static const char *const SHADER_POST_VS = SHADER_SOURCE_VS(
    ...
    uniform vec2 screen; uniform float time;

    void main(void) {
      gl_Position =
          vec4(pos * (vec2(2, -2) / screen.xy) + vec2(-1.0, 1.0), 0.0, 1.0);
      v_uv = uv;
    });
...
static const char *const SHADER_POST_FS_DEFAULT = SHADER_SOURCE_FS(
    ...
    void main(void) { FRAG_COLOR = TEXTURE(screenbuffer, v_uv); });
...
static const char *const SHADER_POST_FS_CRT = SHADER_SOURCE_FS(
    ...
    uniform float time; uniform sampler2D screenbuffer; uniform vec2 screen;
    ...
    void main() {
      ...
      color.r =
          TEXTURE(screenbuffer, vec2(x + uv.x + 0.001, uv.y + 0.001)).x + 0.05;
      color.g =
          TEXTURE(screenbuffer, vec2(x + uv.x + 0.000, uv.y - 0.002)).y + 0.05;
      color.b =
          TEXTURE(screenbuffer, vec2(x + uv.x - 0.002, uv.y + 0.000)).z + 0.05;
      color.r +=
          0.08 * TEXTURE(screenbuffer, 0.75 * vec2(x + 0.025, -0.027) +
                                           vec2(uv.x + 0.001, uv.y + 0.001))
                     .x;
      color.g +=
          0.05 * TEXTURE(screenbuffer, 0.75 * vec2(x - 0.022, -0.020) +
                                           vec2(uv.x + 0.000, uv.y - 0.002))
                     .y;
      color.b +=
          0.08 * TEXTURE(screenbuffer, 0.75 * vec2(x + -0.02, -0.018) +
                                           vec2(uv.x - 0.002, uv.y + 0.000))
                     .z;
      ...
      float scanlines = clamp(
          0.35 + 0.35 * sin(3.5 * time + uv.y * screen.y * 1.5), 0.0, 1.0);
      ...
      color *= 1.0 - 0.65 * vec3(clamp((mod(gl_FragCoord.x, 2.0) - 1.0) * 2.0,
                                       0.0, 1.0));
...
prg_post_t *shader_post_default_init(void) {
  prg_post_t *s = malloc(sizeof(prg_post_t));
...
// static void gl_message_callback(GLenum source, GLenum type, GLuint id, GLenum
// severity, GLsizei len, const GLchar *message, const void *userParam) {
// 	printf("GL: %s\n", message);
// }
...
void render_backend_init(void) {
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                  RENDER_USE_MIPMAPS ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
  ...
  rgba_t white_pixels[4] = {rgba_white(), rgba_white(), rgba_white(),
                            rgba_white()};
...
void render_set_screen(vec2i_t size) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, backbuffer_size.x, backbuffer_size.y,
...
void render_frame_end(void) {
  ...
  quad_buffer[quad_buffer_len] = (quadverts_t){
      .vertices = {
          ...
          {.pos = {screen_size.x, screen_size.y},
           .uv = {1, 0},
           .color = rgba_white()},
...
void render_flush(void) {
  ...
  glBindBuffer(GL_ARRAY_BUFFER, vbo_quads);
  glBufferData(GL_ARRAY_BUFFER, sizeof(quadverts_t) * quad_buffer_len,
               quad_buffer, GL_DYNAMIC_DRAW);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_indices);
...
void render_draw_quad(quadverts_t *quad, texture_t texture_handle) {
  ...
  for (uint32_t i = 0; i < 4; i++) {
    quad_buffer[quad_buffer_len].vertices[i].uv.x =
        (quad_buffer[quad_buffer_len].vertices[i].uv.x + t->offset.x) *
        (1.0 / RENDER_ATLAS_SIZE_PX);
    quad_buffer[quad_buffer_len].vertices[i].uv.y =
        (quad_buffer[quad_buffer_len].vertices[i].uv.y + t->offset.y) *
        (1.0 / RENDER_ATLAS_SIZE_PX);
  }
  quad_buffer_len++;
}
...
void textures_reset(texture_mark_t mark) {
  ...
  for (int i = 0; i < textures_len; i++) {
    uint32_t grid_x =
        (textures[i].offset.x - RENDER_ATLAS_BORDER) / RENDER_ATLAS_GRID;
    uint32_t grid_y =
        (textures[i].offset.y - RENDER_ATLAS_BORDER) / RENDER_ATLAS_GRID;
    uint32_t grid_width =
        (textures[i].size.x + RENDER_ATLAS_BORDER * 2 + RENDER_ATLAS_GRID - 1) /
        RENDER_ATLAS_GRID;
    uint32_t grid_height =
        (textures[i].size.y + RENDER_ATLAS_BORDER * 2 + RENDER_ATLAS_GRID - 1) /
        RENDER_ATLAS_GRID;
...
// Add the border pixels for this texture
  ...
  rgba_t *pb = alloc(sizeof(rgba_t) * bw * bh);

  if (size.x && size.y) {
    // Top border
    for (int32_t y = 0; y < RENDER_ATLAS_BORDER; y++) {
      memcpy(pb + bw * y + RENDER_ATLAS_BORDER, pixels,
             size.x * sizeof(rgba_t));
    }
    ...
    for (int32_t y = 0; y < bh; y++) {
      for (int32_t x = 0; x < RENDER_ATLAS_BORDER; x++) {
        pb[y * bw + x] =
            pixels[clamp(y - RENDER_ATLAS_BORDER, 0, size.y - 1) * size.x];
      }
    }
    ...
    for (int32_t y = 0; y < bh; y++) {
      for (int32_t x = 0; x < RENDER_ATLAS_BORDER; x++) {
        pb[y * bw + x + bw - RENDER_ATLAS_BORDER] =
            pixels[size.x - 1 +
                   clamp(y - RENDER_ATLAS_BORDER, 0, size.y - 1) * size.x];
      }
    }
  ...
  glTexSubImage2D(GL_TEXTURE_2D, 0, x, y, bw, bh, GL_RGBA, GL_UNSIGNED_BYTE,
                  pb);
  free(pb);
#else
  glTexSubImage2D(GL_TEXTURE_2D, 0, x, y, bw, bh, GL_RGBA, GL_UNSIGNED_BYTE,
  ...
  textures[texture_handle.index] = (atlas_pos_t){
      .offset = {x + RENDER_ATLAS_BORDER, y + RENDER_ATLAS_BORDER},
      .size = size};
