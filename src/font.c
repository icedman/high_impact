...
font_t *font(char *path, char *definition_path) {
  ...
  error_if(metrics->len / 7 != expected_chars,
...
int font_line_width(font_t *font, char *text) {
  ...
  for (char *c = text; *c != '\0' && *c != '\n'; c++) {
    if (*c >= font->first_char && *c <= font->last_char) {
      width +=
          font->glyphs[*c - font->first_char].advance + font->letter_spacing;
    }
  }
  return max(0, width - font->letter_spacing);
}

static int font_draw_line(font_t *font, vec2_t pos, char *text,
                          font_align_t align) {
  ...
  for (char *c = text; *c != '\0' && *c != '\n'; c++, char_count++) {
    if (*c >= font->first_char && *c <= font->last_char) {
      ...
      image_draw_ex(font->image, g->pos, g->size, vec2_add(pos, g->offset),
                    g->size, font->color);
