...
#if defined(RENDER_PLAYDATE)
#include "render_playdate.c"
...
void render_cleanup(void) { render_backend_cleanup(); }
...
void render_resize(vec2i_t avaiable_size) {
  ...
  if (RENDER_SCALE_MODE == RENDER_SCALE_NONE) {
    ...
    screen_scale = min(avaiable_size.x / (float)RENDER_WIDTH,
...
vec2i_t render_size(void) { return logical_size; }

void render_push(void) {
  error_if(transform_stack_index >= RENDER_TRANSFORM_STACK_SIZE - 1,
           "Max transform stack size (%d) reached",
           RENDER_TRANSFORM_STACK_SIZE);
  transform_stack[transform_stack_index + 1] =
      transform_stack[transform_stack_index];
...
void render_translate(vec2_t translate) {
  error_if(transform_stack_index == 0,
           "Cannot translate initial transform. render_push() first.");
...
void render_scale(vec2_t scale) {
  error_if(transform_stack_index == 0,
           "Cannot scale initial transform. render_push() first.");
  mat3_scale(&transform_stack[transform_stack_index], scale);
}

void render_rotate(float rotation) {
  error_if(transform_stack_index == 0,
           "Cannot rotate initial transform. render_push() first.");
...
void render_draw(vec2_t pos, vec2_t size, texture_t texture_handle,
  ...
  if (pos.x > logical_size.x || pos.y > logical_size.y || pos.x + size.x < 0 ||
      pos.y + size.y < 0) {
    return;
  }
