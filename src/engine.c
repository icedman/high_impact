...
void engine_load_level(char *json_path) {
  ...
  static json_t *current_level_def = NULL;
  if (current_level_def) {
    json_destroy(current_level_def);
  }
  current_level_def = json;
  ...
  error_if(!entities, "Map has no entities?!");
  ...
  // do not destroy -- as entites reference char* (like name, targets)
  // json_destroy(json);
}
