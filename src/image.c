...
image_t *image(char *path) {
  ...
  image_paths[images_len] = strdup(path);

#ifdef RENDER_PLAYDATE
  char *tmp = strdup(path);
  strcpy(tmp + (strlen(tmp) - 4), ".pdi");
  ...
  LCDBitmap *bitmap = pd->graphics->loadBitmap(tmp, &outErr);
  if (outErr != NULL) {
    pd->system->logToConsole("Error loading image at path '%s': %s", tmp,
                             outErr);
    printf(">error %s!\n", tmp);
  } else {
    int sw, sh;
