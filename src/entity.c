...
#define ENTITIES_LIST_MAX 32 // TODO ... watch for overflow
...
static void entities_separate_on_x_axis(entity_t *left, entity_t *right,
                                        float left_move, float right_move,
                                        float overlap);
static void entities_separate_on_y_axis(entity_t *top, entity_t *bottom,
                                        float top_move, float bottom_move,
                                        float overlap);
...
#define ENTITY_INIT_VTAB(ENUM, NAME)                                           \
  ...
  for (uint32_t i = 0; i < ENTITY_TYPES_COUNT; i++) {
    if (!entity_vtab[i].load) {
      entity_vtab[i].load = noop_load;
    }
    if (!entity_vtab[i].init) {
      entity_vtab[i].init = noop_init;
    }
    if (!entity_vtab[i].settings) {
      entity_vtab[i].settings = noop_settings;
    }
    if (!entity_vtab[i].update) {
      entity_vtab[i].update = entity_base_update;
    }
    if (!entity_vtab[i].draw) {
      entity_vtab[i].draw = entity_base_draw;
    }
    if (!entity_vtab[i].kill) {
      entity_vtab[i].kill = noop_kill;
    }
    if (!entity_vtab[i].touch) {
      entity_vtab[i].touch = noop_touch;
    }
    if (!entity_vtab[i].collide) {
      entity_vtab[i].collide = noop_collide;
    }
    if (!entity_vtab[i].damage) {
      entity_vtab[i].damage = entity_base_damage;
    }
    if (!entity_vtab[i].trigger) {
      entity_vtab[i].trigger = noop_trigger;
    }
    if (!entity_vtab[i].message) {
      entity_vtab[i].message = noop_message;
    }
  }
...
entity_type_t entity_type_by_name(char *type_name) {
  for (uint32_t type = ENTITY_TYPE_NONE + 1; type < ENTITY_TYPES_COUNT;
       type++) {
...
#define COMPARE_POS(a, b) (a->pos.ENTITY_SWEEP_AXIS > b->pos.ENTITY_SWEEP_AXIS)
  ...
  for (int i = 0; i < entities_len; i++) {
    ...
    if (e1->check_against != ENTITY_GROUP_NONE ||
      ...
      for (int j = i + 1;
           j < entities_len && entities[j]->pos.ENTITY_SWEEP_AXIS < max_pos;
           j++) {
        ...
        if (entity_is_touching(e1, e2)) {
          ...
          if ((int)e1->physics >= ENTITY_COLLIDES_LITE &&
              (int)e2->physics >= ENTITY_COLLIDES_LITE &&
              (e1->physics + e2->physics) >=
                  (ENTITY_COLLIDES_ACTIVE | ENTITY_COLLIDES_LITE) &&
              e1->mass + e2->mass > 0) {
...
entity_list_t entities_by_proximity(entity_t *ent, float radius,
                                    entity_type_t type) {
...
entity_list_t entities_by_location(vec2_t pos, float radius, entity_type_t type,
  ...
  for (int i = max(upper_bound, 0); i < entities_len; i++) {
    ...
    if (entity->pos.ENTITY_SWEEP_AXIS + entity->size.ENTITY_SWEEP_AXIS >=
            start_pos &&
        entity != exclude &&
      ...
      float xd =
          entity->pos.x + (entity->pos.x < pos.x ? entity->size.x : 0) - pos.x;
      float yd =
...
entity_ref_t entity_ref(entity_t *self) {
  ...
  return (entity_ref_t){.id = self->id,
...
void entity_base_update(entity_t *self) {
  ...
  vec2_t friction = vec2(min(self->friction.x * engine.tick, 1),
                         min(self->friction.y * engine.tick, 1));
  self->vel = vec2_add(self->vel, vec2_sub(vec2_mulf(self->accel, engine.tick),
...
static void entity_handle_trace_result(entity_t *self, trace_t *t) {
  ...
  if (self->restitution > 0) {
    ...
    if (fabsf(vel_against_normal) * self->restitution >
        ENTITY_MIN_BOUNCE_VELOCITY) {
...
static void entity_resolve_collision(entity_t *a, entity_t *b) {
  float overlap_x = a->pos.x < b->pos.x ? a->pos.x + a->size.x - b->pos.x
                                        : b->pos.x + b->size.x - a->pos.x;

  float overlap_y = a->pos.y < b->pos.y ? a->pos.y + a->size.y - b->pos.y
                                        : b->pos.y + b->size.y - a->pos.y;

  float a_move;
  float b_move;
  if (a->physics & ENTITY_COLLIDES_LITE || b->physics & ENTITY_COLLIDES_FIXED) {
    a_move = 1;
    b_move = 0;
  } else if (a->physics & ENTITY_COLLIDES_FIXED ||
